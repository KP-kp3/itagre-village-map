...
2. **「マップに登録」**（ログイン済み＆`villagers`未作成の場合のみボタン表示）→ 名前・紹介文・プロフィール画像（Discordアイコンが初期値、変更可）を入力して`villagers`行を作成。この時点では位置情報は登録しない＝地図には表示されない。Instagramリンク・誕生日は任意項目として同じフォームで登録できる（`instagram_url`・`birthday`列）
3. **「ピンを設置する」**（マップ登録済みの場合のみ表示、任意）→ 場所名（例:「調布駅」「野川公園」「◯◯ドッグラン」）と座標を登録して初めて地図にピンが表示される

村民ピンに`owner_name`（オーナー名）・住所的な位置情報は持たせない。表示するのは「プロフィール画像・名前・紹介文・場所名・誕生日・Instagramリンク」（`prefecture`は村民ピンには表示しない。スポットのみ`prefecture`を保持・表示する）。誕生日・Instagramリンクは任意項目で、未入力ならポップアップにも表示しない。Instagramリンクは`src/lib/isSafeHttpUrl.ts`でhttp(s)スキームのみ許可してから`<a>`のhrefとして描画する（全員が閲覧できる公開データのため、`javascript:`等の危険なスキームを弾くdefense-in-depth。フォーム送信時とポップアップ表示時の両方でチェックする）。
