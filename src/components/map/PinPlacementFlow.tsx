...
type Props = {
  open: boolean;
  onClose: () => void;
  onSaved: (resident: Resident | null) => void;
};

export default function PinPlacementFlow({ open, onClose, onSaved }: Props) {
  const { user, villager, refreshVillager } = useAuth();
  const isEdit = !!(villager?.lat != null && villager?.lng != null);
  ...
  const handleSave = async () => {
    if (!user || !tempPosition || !placeName.trim() || submitting) return;
    setSubmitting(true);
    setError(null);
    try {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from("villagers")
        .update({ place_name: null, lat: null, lng: null })
        .eq("user_id", user.id);
      if (updateError) throw updateError;
      await refreshVillager();
      onSaved(null);
                  ...
                  <div className="flex flex-col gap-3">
                    <label className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        placeholder="例：調布駅、野川公園、〇〇ドッグラン"
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex-1 rounded-full bg-teal px-4 py-2.5 text-sm font-semibold text-cream shadow-sm transition hover:bg-teal-dark disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        次へ
                      </button>
                    </div>
          ...
          >
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button
                ...
                onClick={handleSave}
                disabled={submitting}
                className="flex-1 rounded-full bg-teal px-4 py-3 text-sm font-semibold text-cream shadow-sm transition hover:bg-teal-dark disabled:cursor-not-allowed disabled:opacity-50"
              >
                {submitting ? "保存中…" : "保存"}
              </button>
            </div>
      ...
      {step === "done" && (
        <div className="fixed inset-0 z-[1420] flex items-end justify-center bg-ink/15 backdrop-blur-sm sm:items-center">
          ...
          >
            ...
            <p className="text-lg font-bold text-ink">ピンを設置しました！</p>
            <button
              type="button"
              onClick={onClose}
