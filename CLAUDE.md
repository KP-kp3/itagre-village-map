...
- `VillageMap.tsx`（一覧用の地図）：`onFound`で`map.flyTo()`に加え、検索結果地点に仮ピン（MapLibreの標準マーカー、村民/スポットピンとは別の見た目）を1本表示する。村民ピン・スポットピンの実データではない検索結果であることが分かるよう、あえてブランドデザインのピンとは差別化している。次に検索するたびに前回の仮ピンは消して置き換える
